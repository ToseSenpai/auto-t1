...
### Multi-MRN Loop Processing Flow

Il sistema implementa un ciclo automatico per processare **tutti gli MRN** dal file Excel in batch sequenziale:

```
┌──────────────────────────────────────────────────────────────────────┐
│  EXCEL LOADED - 5 MRN rows                                            │
│  ["25IT0000012345C0", "25IT0000067890C0", ...]                        │
└────────────────────────────┬─────────────────────────────────────────┘
                             ↓
┌──────────────────────────────────────────────────────────────────────┐
│  COMMON SETUP (eseguito 1 volta)                                      │
├──────────────────────────────────────────────────────────────────────┤
│  1. Login al sistema                                                  │
│  2. Navigate to /cm/declarations                                      │
│  3. Click "Nuova dichiarazione"                                       │
└────────────────────────────┬─────────────────────────────────────────┘
                             ↓
┌──────────────────────────────────────────────────────────────────────┐
│  FOR EACH MRN (mrnIndex = 0; mrnIndex < totalMRNs; mrnIndex++)       │
├──────────────────────────────────────────────────────────────────────┤
│                                                                        │
│  currentMRN = mrnValues[mrnIndex]    // es: "25IT0000012345C0"       │
│  progress = `[${mrnIndex + 1}/${totalMRNs}]`  // es: "[1/5]"         │
│                                                                        │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  STEP 1: Selection Template                                    │  │
│  │  - Click "NCTS Arrival Notification IT"                        │  │
│  │  - Click modello "MX DHL - MXP GTW - DEST AUT"                 │  │
│  │  - Click "OK" conferma                                         │  │
│  │  - Wait for page load                                          │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                             ↓                                          │
...
│  │  - fillMRNField(currentMRN)                                    │  │
│  │  - verifySedeDestinazione() → IT279100                         │  │
│  │  - fillArrivalDateTime() → current time + 1h (Shadow DOM)      │  │
│  │    ├─ Strategia 1: Direct setter su vaadin-date-time-picker   │  │
│  │    └─ Strategia 2: Shadow DOM access (date-picker, time-picker│  │
│  └────────────────────────────────────────────────────────────────┘  │
│                             ↓                                          │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  STEP 3: Submit Declaration                                    │  │
│  │  - clickSendButton() → click #send                             │  │
│  │  - Wait for button visibility & enabled state                  │  │
│  │  - Screenshot post-click                                       │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                             ↓                                          │
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  STEP 4: Auto-Redirect & Reset (se non ultimo MRN)             │  │
│  │  - Sistema auto-redirect a /cm/declarations                    │  │
│  │  - Wait 2000ms per stabilizzazione pagina                      │  │
│  │  - IF (mrnIndex < totalMRNs - 1):                              │  │
│  │      └─→ clickNewDeclaration() → ricomincia ciclo             │  │
│  └────────────────────────────────────────────────────────────────┘  │
│                             ↓                                          │
│  Progress Update: "[1/5] Dichiarazione inviata per MRN: 25IT00..."   │
│                                                                        │
└────────────────────────────┬─────────────────────────────────────────┘
                             ↓
                    ┌─────── NEXT MRN ────────┐
                    │  (ripeti loop per MRN 2) │
                    └──────────────────────────┘
                             ↓
┌──────────────────────────────────────────────────────────────────────┐
│  COMPLETION SUMMARY                                                   │
│  "Automazione completata! Processati 5 MRN con successo."            │
│  Browser rimane aperto per verifica manuale                          │
└──────────────────────────────────────────────────────────────────────┘
```

**Caratteristiche chiave**:
- **Batch sequenziale**: Un MRN alla volta per evitare race conditions
- **Progress tracking**: Ogni step mostra `[X/Y]` formato (es: `[3/5]`)
- **Error resilience**: Se un MRN fallisce, il loop continua (opzionale: implementare continue-on-error)
- **Shadow DOM handling**: Access Vaadin web components con strategie multiple
- **Auto-reset**: Dopo submit, sistema torna automaticamente a "Nuova dichiarazione"
